...
class RWKV_TimeMix(nn.Layer):
    def __init__(self, config, layer_id):
        ...
        self.time_w = self.create_parameter(
            shape=[config.n_head, config.ctx_len],
            dtype=paddle.get_default_dtype(),
            default_initializer=nn.initializer.Constant(1.0),
        )
        self.time_alpha = self.create_parameter(
            shape=[self.n_head, 1, config.ctx_len],
            dtype=paddle.get_default_dtype(),
            default_initializer=nn.initializer.Constant(1.0),
        )
        self.time_beta = self.create_parameter(
            shape=[self.n_head, config.ctx_len, 1],
            dtype=paddle.get_default_dtype(),
            default_initializer=nn.initializer.Constant(1.0),
        )
        ...
        self.register_buffer(
            "mask", paddle.tril(paddle.ones([config.ctx_len, config.ctx_len]))
        )
    ...
    def forward(self, x):
        ...
        wkv = einsum("htu,buhc->bthc", w, kv).reshape(shape=[B, T, -1])
